...
Lab.prototype.neighbours = function(x,y) {
  var neighbours = [];//check walls
	if (x+1 < ww && !this.getCell(x,y).right) neighbours.push(this.getCell(x+1,y+0));
	if (y+1 < hh && !this.getCell(x,y).bottom) neighbours.push(this.getCell(x+0,y+1));
	if (x-1 >= 0 && !this.getCell(x-1,y).right) neighbours.push(this.getCell(x-1,y+0));
	if (y-1 >= 0 && !this.getCell(x,y-1).bottom) neighbours.push(this.getCell(x+0,y-1));
